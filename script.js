...
// ===== Auto-fix stale localStorage paths =====
// If old data with ./assets/ paths is cached, clear it so fresh data.js paths are used
(function purgeStaleCertCache() {
    try {
        const cached = localStorage.getItem('portfolio_certifications');
        if (cached) {
            const certs = JSON.parse(cached);
            const hasStale = certs.some(c => c.image && c.image.includes('./assets/'));
            if (hasStale) {
...
const renderPortfolio = () => {
    ...
    window.renderCertificationsList = () => {
        ...
        // Fix stale ./assets/ paths that may be cached in localStorage
        certs = certs.map(cert => ({
            ...cert,
            image: cert.image ? cert.image.replace('./assets/', './') : cert.image
        }));
                ...
                <div class="cert-card" ${hasValidLink ? `onclick="window.open('${cert.link}', '_blank')"` : ''}>
                    <div class="cert-img-container">
                        <img src="${cert.image}" alt="${cert.title}" class="cert-img"
                             onerror="this.onerror=null; this.src='https://placehold.co/400x220/1a0a0a/e84393?text=Certificate';">
                    </div>
                    <div class="cert-info">
